...
const OrderDetail = ({data,accessToken,props}) => {
    ...
    const Total = ()=>{
        const {deliveryCharge,products} = data;
        let totalAmount = 0;
        ...
        return <Box className="w-[350px] p-4 bg-slate-50 border rounded-md" gap={2}>
          ...
          <Typography className="text-green-700 w-fit bg-green-300 text-center p-1 pl-2 pr-2 rounded-lg">{data.paymentMethod}</Typography> 
            <ChangePaymentStatus accessToken={accessToken} orderId={data._id} props={props} paymentStatus={data.paymentStatus} />
    ...
    <Box className="w-[100vw] mobile:w-[70vw] space-y-3" gap={2}>
           <Box height={15}/>
       ...
       <ChangeOrderStatus orderStatus={data.status} accessToken={accessToken} props={props} orderId={data._id} />
       </Stack>
